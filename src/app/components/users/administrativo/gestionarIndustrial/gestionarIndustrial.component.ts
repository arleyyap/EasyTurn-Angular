...
@Component({
  selector: 'app-gestionarIndustrial',
  templateUrl: './gestionarIndustrial.component.html',
  styleUrls: ['./gestionarIndustrial.component.css']
})
export class gestionarIndusltrialComponent implements OnInit {
  public turnos = [];
  public Email: any;
  ...
  ngOnInit() {
    this.fsService.getTurnosIndustrial().subscribe((turnosSnapshot) => {
      this.turnos = [];
  ...
  DeleteToken(email: string, TurnoId: string) {
    this.dataApi.searchIndustrialForEmail(email)
      .pipe(map(docArray => {
        return docArray.map(doc => {
          let id = doc.payload.doc.id;
          this.fsService.deleteTurnoIndustrialToken(id).then(() => {
    ...
    this.fsService.deleteTurnoIndustrial(TurnoId).then(() => {
